...
namespace ZXing.Mobile
{
	...
	public class ZXingSurfaceView : SurfaceView, ISurfaceHolderCallback, Android.Hardware.Camera.IPreviewCallback, Android.Hardware.Camera.IAutoFocusCallback
	{
		...
		public ZXingSurfaceView(Activity activity, MobileBarcodeScanningOptions options, Action<ZXing.Result> callback) : base(activity)
		{
			CheckPermissions();
		...
		public void SurfaceCreated(ISurfaceHolder holder)
		{
			...
			try
			{
				...
				if(version >= BuildVersionCodes.Gingerbread)
				{
					...
					if(options.UseFrontCameraIfAvailable.HasValue && options.UseFrontCameraIfAvailable.Value) whichCamera = CameraFacing.Front;
				...
				if(camera == null) Android.Util.Log.Debug("ZXing.Net.Mobile", "Camera is null :(");
		...
		public void OnAutoFocus(bool success, Android.Hardware.Camera camera)
		{
			...
			System.Threading.Tasks.Task.Factory.StartNew(() =>{
				...
				if(!tokenSource.IsCancellationRequested) AutoFocus();
			});
		}
		...
		public void Torch(bool on)
		{
			...
			if(supportedFlashModes == null) supportedFlashModes = new List<string>();
			
			var flashMode = string.Empty;
			
			if(on)
			{
				if(supportedFlashModes.Contains(Android.Hardware.Camera.Parameters.FlashModeTorch)) flashMode = Android.Hardware.Camera.Parameters.FlashModeTorch;
				else if(supportedFlashModes.Contains(Android.Hardware.Camera.Parameters.FlashModeOn)) flashMode = Android.Hardware.Camera.Parameters.FlashModeOn;
			} else
			{
				if(supportedFlashModes.Contains(Android.Hardware.Camera.Parameters.FlashModeOff)) flashMode = Android.Hardware.Camera.Parameters.FlashModeOff;
			}
		...
		private void drawResultPoints(Android.Graphics.Bitmap barcode, ZXing.Result rawResult)
		{
			...
			if(points != null && points.Length > 0)
			{
				...
				} else
				{
					...
					foreach(ResultPoint point in points) canvas.DrawPoint(point.X, point.Y, paint);
				}
			}
		}
